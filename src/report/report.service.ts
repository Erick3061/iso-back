...
export class ReportService {
  ...
  async Srs_Sta({ start, end }: DateDto) {
    try {
      ...
      Data = Data.map((account) => {
        const events = this.filtersHelpers.deleteRepeat({
          data: this.filtersHelpers.deleteRepeat({
            data: account.eventos,
            Alarm: 'CodigoAlarma',
            Event: 'SRS',
          }),
          Alarm: 'CodigoAlarma',
      ...
      const sta = [];
      const filters: Array<
        { CodigoCte: string; CodigoAbonado: string } & Pick<
          (typeof Data)[0],
          'eventos'
        >
      > = [];
      ...
      Data.forEach((data) => {
        ...
        if (
          ...
          data.eventos[0].CodigoAlarma === 'SRS'
        ) {
          ...
          if (data.eventos.length !== 1) {
            ...
            data.eventos.forEach((ev, idx) => {
              if (CC !== data.CodigoCte) {
                ...
                if (ban === false && BEA2 === true && ev.CodigoAlarma === 'SRS')
                  BEA2 = false;
              }
            });

            if (
              D.filter((s) => s.CodigoAlarma && s.CodigoAlarma !== 'STA')
                .length > 0
            ) {
              D.filter((s) => s.CodigoAlarma && s.CodigoAlarma !== 'STA').map(
                (event) => {
                  filters.push({
                    ...event,
                    CodigoCte: data.CodigoCte,
  ...
  async Tess_Tese({ start, end }: DateDto) {
    try {
      ...
      const events = data
        .filter((account) => account.eventos)
        .map((account) => {
          const evs = account.eventos.map((ev) => {
            const comment = account.comentarios.find(
              (com) =>
                `${com.FechaEvento} ${com.HoraEvento}` ===
                `${ev.FechaOriginal} ${ev.Hora}`,
            );
            if (comment) {
              return {
      ...
      const tess = events.filter(
        (el) =>
          el.CodigoAlarma === 'TESS' &&
          el.Comment.toLowerCase().includes('folio') &&
          !el.Comment.toLowerCase().includes('sin folio'),
      );
      const tese = events.filter(
        (el) =>
          el.CodigoAlarma === 'TESE' &&
          (el.Comment.toLowerCase().includes('tec') ||
            el.Comment.toLowerCase().includes('téc')),
      );
  ...
  async At5ma({ start, end }: DateDto) {
    try {
      ...
      const events = data.map((event) => {
        return {
          ...event,
          Minutes: this.filtersHelpers.getMinutes(
            new Date(
              `${event.FechaPrimeraToma}T${event.HoraPrimeraToma}.000Z`,
            ).getTime() -
              new Date(`${event.FechaOriginal}T${event.Hora}.000Z`).getTime(),
          ).minutes,
        };
      });
  ...
  async AtOperator({ start, end }: DateTimeDto) {
    try {
            ...
            return {
              ...
              Minutes: this.filtersHelpers.getMinutes(
                new Date(
                  ...
                  new Date(
                    `${event.FechaOriginal}T${event.Hora}.000Z`,
                  ).getTime(),
              ).minutes,
            };
        })
      ...
      const operators = [
        ...new Set(events.map((event) => event.ClaveMonitorista)),
      ].reduce(
        (acc, current) => [
          ...acc,
          {
            ...
            events: events.filter(
              (event) => event.ClaveMonitorista === current,
            ),
          },
        ],
        [],
  ...
  private handleError(error: any) {
    if (error.code === 2 && error.details.includes('NO EXISTE EN EL SERVIDOR'))
