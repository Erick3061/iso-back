...
export class UserService {
  ...
  async findOne(term: string, msgError?: string) {
    ...
    if (!user)
      throw msgError
        ? new UnauthorizedException(msgError)
        : new NotFoundException(`user with ${term} not found`);
    return user;
  }
  ...
  async remove(id: string, admin: User) {
    try {
      ...
      if (admin.id === id)
        throw new BadRequestException(`User ${admin.fullName} are not valid`);
  ...
  private handleError(error: any): never {
    if (error.errno === 19 && error.code === 'SQLITE_CONSTRAINT') {
      if (`${error.driverError}`.includes('UNIQUE constraint failed')) {
        const message: string = `${error.driverError}`
          .replace('SQLITE_CONSTRAINT:', '')
          .replace('UNIQUE constraint failed:', '');
      ...
      if (`${error.driverError}`.includes('FOREIGN KEY constraint failed')) {
        throw new BadRequestException(
          `Invalid action, depends on other values.`,
        );
      }
